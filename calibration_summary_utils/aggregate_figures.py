...
def summarize_all(summaries_dir: Path,
    ...
    if not summary_files:
        raise FileNotFoundError(f"No JSON files found in the directory {summaries_dir}.")
    for file in tqdm(summary_files):
        with open(file, 'r') as f:
            summary = json.load(f)
        max_points_benign = -1 * max_points[5 - np.isnan(summary['final_benign_thresholds']).sum() ]
        max_points_pathogenic = max_points[5 - np.isnan(summary['final_pathogenic_thresholds']).sum() ]
    ...
    if output_summary_file is not None:
       summaries.to_excel(output_summary_file, index_label='Dataset', sheet_name='Calibration Summary')
    pivot_table = summaries.pivot_table(index='max_points_benign', 
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Visualize summary of fit results from a JSON file.")
    parser.add_argument('summaries_dir', type=Path, help='Path to the summary JSON file.')
    parser.add_argument('--output', type=Path, default=None, help='Output file for the summary (optional).')
    parser.add_argument('--output_image_file', type=Path, default=None, help='Output file for the visualization (optional).')
