...
def select_final_models(all_models_file: Path, final_models_file: Path, num_models: int = 2000):
...
def aggregate_and_select_final_models(models_dir: str|Path,final_models_file: str|Path, num_models: Optional[int|None] = None):
    ...
    for k, v in all_models.items():
        if num_models is not None and len(v) < num_models:
            raise ValueError(f"Not enough models for dataset {k}. Found {len(v)}, expected at least {num_models}.")
    final_models = {}
    if num_models is not None:
        final_models = {k : random.sample(v, num_models) for k,v in all_models.items()}
    with open(final_models_file, 'w') as out_file:
        json.dump(final_models if num_models is not None else all_models, out_file, indent=4)
    
if __name__ == "__main__":
    def main():
        parser = argparse.ArgumentParser(description="Aggregate models and select a subset of final models.")
        parser.add_argument("models_dir", type=str, help="Directory containing model JSON files.")
        parser.add_argument("final_models_file", type=str, help="Output file for the final models.")
        parser.add_argument("--num_models", type=int, default=None, help="Number of models to select for each dataset. Defaults to all models.")
        
        args = parser.parse_args()
        aggregate_and_select_final_models(args.models_dir, args.final_models_file, args.num_models)
