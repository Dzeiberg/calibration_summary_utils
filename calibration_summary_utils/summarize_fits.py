...
from pillar_project.model_selection.model_selection_results import get_priors, get_thresholds,summarize_thresholds
import json
import numpy as np
from tqdm import tqdm
from select_final_models import aggregate_and_select_final_models
from visualize_fits import visualize_fits
...
def summarize_fits(models_dir: Path,
                   scoresets_dir: Path,
    ...
    print(f"Aggregating and selecting final models from {models_dir} to {final_models_filepath}")
    aggregate_and_select_final_models(models_dir, final_models_filepath, num_models=None)
    ...
    for scoreset_name, fit_dicts in tqdm(models.items(), desc="Summarizing scoresets",total=len(models)):
        ...
        if scoreset_summary_file.exists():
            ...
            continue
        subset = pillar_df[pillar_df.Dataset == scoreset_name].drop_duplicates(subset=['ID'])
        if subset.empty:
            raise ValueError(f"No data found for scoreset {scoreset_name} in the dataframe.")
        ...
        pathogenic_threshold_sets, benign_threshold_sets = get_thresholds(fits, median_prior, inverted,)
        ...
        final_pathogenic_thresholds, final_benign_thresholds = summarize_thresholds(pathogenic_threshold_sets,
                                                                                       benign_threshold_sets,
                                                                                       final_threshold_quantile,
                                                                                       inverted)
        scoreset_result['final_pathogenic_thresholds'] = final_pathogenic_thresholds.tolist()
    ...
    visualize_fits(scoresets_dir=scoresets_dir,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Summarize fits from models and scoresets.")
    parser.add_argument('models_dir', type=Path, help='Path to the models JSON file.')
    parser.add_argument('scoresets_dir', type=Path, help='Directory containing the scoresets.')
    parser.add_argument("final_models_filepath", type=Path, help='Path to which the final models file will be saved.')
    parser.add_argument('summaries_dir', type=Path, help='Output directory for the fit summaries.')
    parser.add_argument('figures_dir', type=Path, help='Output directory for the fit visualizations.')
    parser.add_argument('dataframe_filepath', type=Path, help='Path to the pillar dataframe CSV file.')
    parser.add_argument('--final_threshold_quantile', type=float, default=0.05, help='Quantile for final thresholds (default: 0.05)')
    args = parser.parse_args()
    
    summarize_fits(models_dir=args.models_dir,
    ...
    # Example usage:
        # python scripts/summarize_fits.py \
        # /path/to/models \
        # /path/to/scoresets \
        # /path/to/save/final_models.json 
        # /path/to/save/summaries \
        # /path/to/save/figures \
        # /path/to/pillar_dataframe.csv \
